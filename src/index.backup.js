...
function drawBackgroundShapes() {
  ...
  const makeBlob = (cx, cy) => {
    const rx = w * brush.random(0.04, 0.17);
    const ry = h * brush.random(0.04, 0.17);
  ...
  const mapSym = ({ pts, cx: scx, cy: scy, span }, px, py) => {
    const s = (Math.min(w, h) * brush.random(0.1, 0.27)) / span;
    const angle = brush.random(-Math.PI / 20, Math.PI / 20);
    const cos = Math.cos(angle), sin = Math.sin(angle);
    return pts.map(([x, y]) => {
      const dx = (x - scx) * s, dy = (y - scy) * s;
...
// Decorative architectural symbols — cross-mark and zigzag chevron
const archSym1 = [
  ...
  [499, 983],[484, 984],[469, 980],[470, 965],[470, 950],[470, 935],
  [471, 920],
];
const archSym2 = [
...
// Centroid and reference span for each symbol
const archSyms = [
...
function drawComposedScene() {
  ...
  for (const { pts, cx: scx, cy: scy, span } of archSyms) {
    if (brush.random(1) > 0.5) {
      const s = (Math.min(w, h) * brush.random(0.1, 0.26)) / span;
      const px = brush.random(skyX0, skyX1);
      const py = brush.random(skyY0, skyY1);
      const angle = brush.random(-Math.PI / 20, Math.PI / 20);
      const cos = Math.cos(angle), sin = Math.sin(angle);
      const symPts = pts.map(([x, y]) => {
        const dx = (x - scx) * s, dy = (y - scy) * s;
        return [px + cos * dx - sin * dy, py + sin * dx + cos * dy];
      });
      fillShape(symPts, sunColor);
    }
  }
...
const composedBg = brush.random(1) > 0.8;
const singleHand = brush.random(1) > 0.8;
